...
function getNextFriday(date) {
  ...
  let differents;
  if (currentDay < 5) {
    differents = 5 - currentDay;
  } else {
    differents = 12 - currentDay;
  }
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const different = Date.parse(dateEnd) - Date.parse(dateStart);
  return different / 1000 / 3600 / 24 + 1;
}
...
function formatDate(date) {
  ...
  const time = currentDate.toLocaleTimeString('en', { timeZone: 'UTC' });
...
function getWeekNumberByDate(date) {
  ...
  const dayCount = (date - yearStart) / 1000 / 3600 / 24 + 1;
  let weekCount = 0;

  for (let i = 1; i < dayCount + 1; i += 1) {
    if (new Date(Date.UTC(year, 0, i)).getUTCDay() === 0) {
      weekCount += 1;
    }
  }
...
function getNextFridayThe13th(date) {
  ...
  let newDate;
  let is13Friday = false;

  while (!is13Friday) {
    newDate = new Date(currentYear, currentMonth, 13);
    is13Friday = newDate.getDay() === 5;
    currentMonth += 1;
  }
